...
# 3.4 Validar que nginx config es válido
$allValid = $allValid -and (Test-Step `
        -Name "Nginx config válido (nginx -t)" `
        -Test {
        $result = Invoke-SSHCommand "nginx -t 2>&1"
        return $result -like "*test is successful*"
    } `
        -SuccessMessage "Sintaxis nginx OK" `
        -ErrorMessage "Error en configuración nginx"
)
...
if (-not $SkipTests) {
    ...
    $allValid = $allValid -and (Test-Step `
            ...
            -Test {
            $result = Invoke-SSHCommand "curl -s -o /dev/null -w '%{http_code}' ${DOMAIN}/"
            return $result.Trim() -eq "200"
        } `
