...
def parse_fila(linea):
    ...
    montos = [parse_precio(t) for t in tokens if re.fullmatch(MONEY, t)]
    if montos:
        # último monto = penalidad; primeros 3 = instalación; siguientes 3 = activación
        if len(montos) >= 7:
            extras["instalacion"] = {"0m": montos[0], "12m": montos[1], "24m": montos[2]}
            extras["activacion"] = {"0m": montos[3], "12m": montos[4], "24m": montos[5]}
            extras["penalidad"] = montos[6]
        elif len(montos) >= 4:
            extras["instalacion"] = {"0m": montos[0], "12m": montos[1], "24m": montos[2]}
            extras["penalidad"] = montos[-1]
        else:
            extras["penalidad"] = montos[-1]
    if "ILIM" in resto:
        extras["minuto_adicional"] = "ILIM"
    elif "$0.03" in resto:
        extras["minuto_adicional"] = 0.03
