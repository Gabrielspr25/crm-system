...
export default function Reports() {
  ...
  // Helper: Calcular comisión empresa según producto
  const calculateCompanyEarnings = (productName: string, amount: number): number => {
    if (amount <= 0) return 0;

    const lowerName = productName.toLowerCase();

    // MÓVIL NEW/REN: Usar tiers (comisión fija por línea según precio)
    if (lowerName.includes('movil')) {
      if (!products || !tiers) return 0;
      const product = products.find(p => p.name.toLowerCase() === lowerName);
      if (!product) return 0;
    ...
    // OTROS PRODUCTOS: Usar commission_percentage de la tabla products
    if (!products) return 0;
    const product = products.find(p => p.name.toLowerCase() === lowerName);
    if (!product || !product.commission_percentage) return 0;

    return amount * (product.commission_percentage / 100);
  };

  // Helper: Calcular comisión del vendedor (usa % del vendedor desde BD)
  const calculateVendorCommission = (productName: string, amount: number, vendorPct: number): number => {
    if (amount <= 0) return 0;

    const companyEarnings = calculateCompanyEarnings(productName, amount);
    return companyEarnings * (vendorPct / 100);
  };
  ...
  const salesRows = useMemo(() => {
    ...
    filteredProspects.forEach((prospect) => {
      ...
      // Comisión vendedor (usa % del vendedor desde BD)
      const vendorCommissions = {
        fijo_ren: calculateVendorCommission('Fijo Ren', fijoRenValue, vendorCommissionPct),
        fijo_new: calculateVendorCommission('Fijo New', fijoNewValue, vendorCommissionPct),
        movil_nueva: calculateVendorCommission('Movil New', movilNuevaValue, vendorCommissionPct),
        movil_ren: calculateVendorCommission('Movil Ren', movilRenValue, vendorCommissionPct),
        claro_tv: calculateVendorCommission('Claro TV', claroTvValue, vendorCommissionPct),
        cloud: calculateVendorCommission('Cloud', cloudValue, vendorCommissionPct),
        mpls: calculateVendorCommission('MPLS', mplsValue, vendorCommissionPct)
      };
      const totalVendorCommission = Object.values(vendorCommissions).reduce((sum, val) => sum + val, 0);
