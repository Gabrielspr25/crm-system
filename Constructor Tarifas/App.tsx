...
import { LayoutGrid, UploadCloud, MessageSquare, Phone, Wifi, MonitorPlay, Smartphone, CheckCircle2, Circle, Star, Sparkles, Zap, Settings, Save, Trash2, Plus, FileText, ChevronRight, Layout, History, AlertCircle, Clock, CheckCircle, Database, Calendar, Calculator as CalcIcon } from 'lucide-react';
...
import { fetchPlans, updatePlan, createPlan } from './services/tariffsApi';

function App() {
  const [currentView, setCurrentView] = useState<ViewState>('catalog');
  // Initial state is empty, loaded via effect
  ...
  const resetSystem = () => {
    if (confirm("¿Deseas borrar toda la data actual y volver a las estructuras base en blanco para empezar la carga de documentos?")) {
      ...
      setFileHistory([]);
      setPendingChanges([]);
      loadPlans(); // Reload from DB or clear DB logic if implemented
  ...
  const saveCurrentPlan = async () => {
    ...
    try {
      // Si tiene ID numérico (venía de DB), actualizamos. Si es nuevo, creamos (lógica simplificada)
      await updatePlan(editingPlan.id, editingPlan);
      alert("✅ Plan guardado exitosamente en Base de Datos");
      loadPlans(); // Reload to ensure sync
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex flex-col h-screen">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* PANEL DERECHO (AHORA IZQUIERDO): MESA DE OPERACIONES */}
        {showSidebar && (
          <aside className="w-96 bg-slate-900 border-r border-slate-800 flex flex-col shadow-2xl z-40">
              ...
              {editingPlan ? (
                <div className="bg-slate-900 rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl">
                  <div className="bg-slate-800/40 p-8 flex items-center justify-between border-b border-slate-800">
                    ...
                    <button onClick={saveCurrentPlan} className="bg-emerald-600 hover:bg-emerald-700 text-white px-10 py-4 rounded-2xl font-black flex items-center gap-3 transition-all shadow-xl shadow-emerald-500/20 active:scale-95">
...
// Arrow helper missing in some versions
        ...
        {/* Floating Calculator Button */}
        {
          !showCalculator && (
            <button
              ...
              className="fixed bottom-6 right-6 bg-emerald-600 hover:bg-emerald-500 text-white p-4 rounded-full shadow-2xl shadow-emerald-500/30 transition-all active:scale-95 z-50 flex items-center justify-center animate-in zoom-in"
            >
              <CalcIcon size={24} />
            </button>
          )
        ...
        {showCalculator && <Calculator plans={plans} onClose={() => setShowCalculator(false)} />}
      </div >
      );
}
